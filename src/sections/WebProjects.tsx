...
const ProjectCard: React.FC<{
  ...
  <button
    className={`project-card group text-left w-full reveal reveal-delay-${delay}${inView ? ' visible' : ''}`}
    onClick={onClick}
    aria-label={`Ver detalhes: ${project.name}`}
  >
    {/* Media */}
    <div className="overflow-hidden" style={{ aspectRatio: featured ? '16/8' : '16/10' }}>
      {project.preview?.endsWith('.mp4') ? (
        <video src={project.preview} autoPlay muted loop playsInline aria-hidden="true"
          className="w-full h-full object-cover block transition-transform duration-500 group-hover:scale-105" />
      ) : project.preview ? (
        <img src={project.preview} alt="" aria-hidden="true" loading="lazy"
    ...
    <div className={featured ? 'p-6 md:p-7' : 'p-5'}>
      ...
      <h3 className={`font-bold leading-tight mb-2 ${featured ? 'text-xl md:text-2xl' : 'text-lg'}`}
        style={{ color: '#F3F4F6' }}>
        {project.name}
      </h3>
      <p className={`leading-relaxed ${featured ? 'text-base' : 'text-sm line-clamp-2'}`}
        style={{ color: '#94A3B8' }}>
        {project.description}
      </p>
      <div className="flex items-center gap-1.5 mt-4 text-sm font-semibold" style={{ color: '#60A5FA' }}>
...
const WebProjects: React.FC = () => {
  ...
  return (
    <section id="web-projects" className="py-24 md:py-32" style={{ background: '#081225' }} aria-label="Projectos Web">
      <div className="max-w-6xl mx-auto px-5 md:px-8">
        ...
        {featured && (
          <div className={`mb-4 reveal reveal-delay-1${inView ? ' visible' : ''}`}>
            <ProjectCard project={featured} onClick={() => setSelected(featured)} featured delay={1} inView={inView} />
          </div>
        )}
